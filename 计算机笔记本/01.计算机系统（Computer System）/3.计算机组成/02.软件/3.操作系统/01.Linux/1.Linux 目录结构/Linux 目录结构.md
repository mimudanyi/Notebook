...
- /etc：这个目录用来存放所有的系统管理所需要的配置文件和子目录。更改了该目录下的某个文件可能会导致系统不能启动。
- /lib：这个目录里存放着系统最基本的动态连接共享库（比如c++库），其作用类似于Windows里的DLL文件。几乎所有的应用程序都需要用到这些共享库。
...
- /media：类windows的其他设备，例如U盘、光驱等等，linux     系统会自动识别一些设备，例如U盘、光驱等等，当识别后，linux会把识别的设备挂载到这个目录下。
